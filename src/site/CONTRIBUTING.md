...
正文标题用一级，章节用二级、三级。段落之间空一行，列表、引用块、代码块各自独立成段，代码块标注语言。表格用 GFM 语法：必须有分隔行，否则按普通文本渲染。

渲染由 `react-markdown` 加 `remark-gfm` 负责，支持标题、列表（含嵌套）、引用块、代码块、GFM 表格、水平线、自动链接、删除线、任务列表，以及行内加粗、斜体、行内代码与链接。代码高亮与智能引号不在默认范围，需要时另加插件。

原始 HTML 默认不作为元素解析，内容不经 `dangerouslySetInnerHTML`；这不代替来源把关，`data/` 内容仍须来自可信来源。
...
路由新增或变更时，在 `src/App.test.tsx` 的 routes 表增删对应行；渲染相关的变更补 `src/components/Markdown.test.tsx`；元数据解析与剥离的变更补 `src/utils/frontmatter.test.ts`；学习资料索引与内容约定的变更补 `src/models/learning.test.ts`。测试环境为 jsdom 加 Testing Library，配置见 `vitest.config.ts`。
