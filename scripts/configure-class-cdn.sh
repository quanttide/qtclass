#!/bin/bash
# 为 class.quanttide.com 配置 CDN HTTPS 证书 + DNS CNAME。
#
# 背景：class.quanttide.com 是 qtclass 客户端发布域名，CDN 源站为 OSS 桶 qtclass-studio
...
EXISTING=$(aliyun alidns DescribeDomainRecords --DomainName quanttide.com --RRKeyWord class --ValueKeyWord "${DOMAIN}.w.kunlunaq.com" 2>/dev/null | python3 -c "
import sys, json
try:
